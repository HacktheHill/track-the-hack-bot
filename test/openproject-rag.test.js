...
test("AI evaluation caches count only real extracted-task predictions as valid", () => {
	...
	assert.equal(validEvaluationCacheEntry(JSON.stringify({ version: "automatic-v3.9", predicted: [task], trace })), true);
	assert.equal(validEvaluationCacheEntry(JSON.stringify({ version: "automatic-v3.9", predicted: [{ title: "partial" }], trace })), false);
...
test("automatic user catalog uses active and invited non-group accounts instead of project assignees", async () => {
	const originalFetch = globalThis.fetch;
	const paths = [];
	globalThis.fetch = async url => {
	...
	try {
		const client = new OpenProjectClient({ OPENPROJECT_BASE_URL: "https://openproject.example", OPENPROJECT_API_KEY: "secret", OPENPROJECT_CACHE_TTL_MS: 1000, teamRoles: { role: { projectId: 9 } } });
		assert.deepEqual((await client.users()).map(user => user.id), [1, 2]);
		assert.deepEqual(paths, ["/api/v3/users"]);
	} finally {
		globalThis.fetch = originalFetch;
	}
});
