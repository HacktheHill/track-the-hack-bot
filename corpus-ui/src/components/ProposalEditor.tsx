	...
	return <section aria-labelledby="expected-title">
		...
		<div className="proposal-stack">
			{proposals.map((proposal, index) => <fieldset className="proposal" key={index}>
				...
				<label>Title must include<input id={`proposal-title-${index}`} value={proposal.titleIncludes.join(", ")} onChange={event => patch(index, { titleIncludes: event.target.value.split(",").map(value => value.trim()).filter(Boolean) })} placeholder="sponsor, outreach, deck" aria-invalid={invalidFields.includes(`proposal-title-${index}`)} aria-describedby={`title-help-${index}`} /></label>
