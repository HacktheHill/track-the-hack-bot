...
export class Database {
	...
	async supersedePendingProposalsForDeletedSource(messageId: string) {
		// ⚡ Bolt: Batching update and insert into a single CTE minimizes roundtrips
		// and transaction time, avoiding an N+1 query pattern for the audit log inserts.
		...
		const result = await this.pool.query<{ id: string; channel_id: string; review_message_id: string | null }>(
			...
			audit_log_inserts AS (
				INSERT INTO task_audit_log(proposal_id,event,metadata)
				SELECT id,'source_deleted',$2 FROM updated_proposals
			)
			SELECT id,channel_id,review_message_id FROM updated_proposals`,
	...
	async supersedePendingProposalForInvalidSources(id: string, missingMessageIds: string[], contentChanged = false) {
		...
		const result = await this.pool.query(
			...
			audit_log_inserts AS (
				INSERT INTO task_audit_log(proposal_id,event,metadata)
				SELECT id,'source_invalid_preflight',$3 FROM updated_proposals
			)
			SELECT id FROM updated_proposals`,
