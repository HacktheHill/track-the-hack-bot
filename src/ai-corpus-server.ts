...
function timingSafeEqualString(a: string | undefined, b: string | undefined) {
...
export function createCorpusApp(options: {
	...
	app.use("/api", (request, response, next) => {
		if (!timingSafeEqualString(sessionToken(request), options.token)) return response.status(401).send("Invalid corpus session.");
		next();
	});
	...
	app.get("/", async (_request, response, next) => {
		try {
			...
			if (!timingSafeEqualString(sessionToken(request), options.token)) {
				if (!timingSafeEqualString(typeof request.query.token === "string" ? request.query.token : undefined, options.token)) return response.status(401).send("Open the one-time URL printed by the corpus UI process.");
