...
function secureCompare(a: string | undefined, b: string) {
...
export function createCorpusApp(options: {
	...
	app.use("/api", (request, response, next) => {
		if (!secureCompare(sessionToken(request), options.token)) return response.status(401).type("text").send("Invalid corpus session.");
		next();
	});
	...
	app.get("/", async (_request, response, next) => {
		try {
			...
			if (!secureCompare(sessionToken(request), options.token)) {
				const queryToken = typeof request.query.token === "string" ? request.query.token : undefined;
				if (!secureCompare(queryToken, options.token)) return response.status(401).type("text").send("Open the one-time URL printed by the corpus UI process.");
