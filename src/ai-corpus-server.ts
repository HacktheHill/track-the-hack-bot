...
export function createCorpusApp(options: {
	...
	app.use((request, response, next) => {
		if (!localAuthority(request.headers.host)) return response.type("text").status(403).send("Localhost access only.");
		if (request.headers.origin && !localAuthority(request.headers.origin)) return response.type("text").status(403).send("Cross-origin requests are not allowed.");
	...
	app.use("/api", (request, response, next) => {
		if (sessionToken(request) !== options.token) return response.type("text").status(401).send("Invalid corpus session.");
		next();
	});
	...
	app.post("/api/cases/:id/reconstruction-preview", async (request, response, next) => {
		try {
			if (!options.recoverContext) return response.type("text").status(503).send("Discord context recovery is not configured for this local review desk.");
			const input = recoveryRequestSchema.parse(request.body);
			const current = await options.store.getCase(request.params.id);
			if (current.etag !== input.etag) return response.type("text").status(409).send("This case changed after it was opened. Reload before recovering context.");
	...
	app.put("/api/cases/:id", async (request, response, next) => {
		try {
			...
			if (input.case.id !== request.params.id) return response.type("text").status(400).send("Case ID cannot be changed.");
	...
	app.get("/", async (_request, response, next) => {
		try {
			...
			if (sessionToken(request) !== options.token) {
				if (request.query.token !== options.token) return response.type("text").status(401).send("Open the one-time URL printed by the corpus UI process.");
	...
	app.use((error: unknown, _request: Request, response: Response, _next: unknown) => {
		...
		// SECURITY: Prevent Reflected XSS by explicitly setting the Content-Type to plain text
		response.type("text").status(safeStatus).send(safeStatus === 500 ? "Corpus operation failed. Check the local terminal for the error category." : detail);
