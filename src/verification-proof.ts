...
export const readDiscordProof = (
	...
	if (expires <= seconds || expires > seconds + DISCORD_LINK_TTL_SECONDS + 30)
		return null;
...
export const signDiscordRequest = (
	body: string,
	timestamp: string,
	secret: string,
) =>
	createHmac("sha256", secret)
		.update(`discord-complete:v1:${timestamp}.${body}`)
		.digest("hex");
...
export const signNotificationRequest = (
	...
	createHmac("sha256", secret)
		.update(`discord-${domain}:v1:${timestamp}.${body}`)
		.digest("hex");
