			...
			body: JSON.stringify({
				...
				messages: [
					{ role: "system", content: [
						...
						"Set is_durable=true only when an asynchronous tracker remains useful after the live exchange. Choosing a meeting time, sending a routine calendar invitation, arranging attendance or an in-person handoff, access-code and login assistance, immediate help already being handled, and other synchronous coordination are false. A separately assigned agenda, briefing, revision, decision document, or other durable meeting-related artifact may be true.",
						"Set is_decision_ready=true only when the desired outcome is sufficiently decided. Questions about how a process works, who should perform it, whether to proceed, or which mutually exclusive method to use are false until the discussion resolves the choice. If a request depends on an unidentified object or missing conversation and materially different tasks could fit, it is false. A question checking readiness, blockers, or available inputs remains decision-ready when the same context clearly requires a deliverable. A request to review a concrete artifact is decision-ready, while merely sharing the artifact is not.",
						"Accepted review feedback with a commitment to revise has activated remaining work even when the original feedback was phrased as questions. A focal continuation or acknowledgement may activate or reaffirm work through its reply chain when that chain contains the concrete assignment; cite both the focal continuation and the pivotal assignment evidence.",
						...
						"Examples: 'Can you publish this reel?' passes activation; 'How does Instagram access work?' fails decision readiness; 'Alice will publish tomorrow' passes; 'Reach out if you need tasks or support' fails activation; 'Join the meeting now' fails durability; 'Here are the tasks already in the tracker' fails activation; 'Please address these accepted comments in the next revision' passes.",
					].join(" ") },
					{ role: "user", content: [
